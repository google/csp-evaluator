...
#
# @fileoverview Shell script to download csp-evaluator build dependencies
#
...
type ant >/dev/null 2>&1 || {
  echo >&2 "Ant is required to build csp-evaluator dependencies."
  exit 1
}
type javac >/dev/null 2>&1 || {
  echo >&2 "Java compiler is required to build csp-evaluator dependencies."
  exit 1
}
type git >/dev/null 2>&1 || {
  echo >&2 "Git is required to build csp-evaluator dependencies."
  exit 1
}
type curl >/dev/null 2>&1 || {
  echo >&2 "Curl is required to build csp-evaluator dependencies."
  exit 1
}
type unzip >/dev/null 2>&1 || {
  echo >&2 "Unzip is required to build csp-evaluator dependencies."
  exit 1
}
